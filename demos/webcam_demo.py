...
if __name__ == '__main__':
    ...
    dataset = LoadStreams(args.source, img_size=imgsz, stride=stride, auto=True)
    ...
    gif_frames = []
    # out_name = '{}_{}_{}'.format(osp.splitext(args.source)[0], osp.splitext(args.weights)[0],
    ...
    dataset = tqdm(dataset, desc='Running inference')
    t0 = time_sync()
    for i, (path, img, im0, _) in enumerate(dataset):
        ...
        bboxes, poses, _, _, _ = post_process_batch(data, img, [], [[im0[0].shape[:2]]], person_dets, kp_dets)
        ...
        im0 = cv2.addWeighted(im0[0], args.alpha, im0_copy, 1 - args.alpha, gamma=0)
        ...
        if args.gif:
            ...
            cv2.putText(gif_img, '{:.1f} FPS'.format(1 / t), (5, 25),
